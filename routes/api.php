...
Route::namespace('Auth')->prefix('auth')->name('auth.')->group(function () {
    ...
    Route::post('login', 'AuthController@login')->middleware('throttle:30,1')->name('login');
...
Route::namespace('User')->middleware('auth:api')->prefix('users/me')->name('users.me.')->group(function () {
    Route::post('keys/{key}', 'KeyController@show')->middleware('throttle:30,1')->name('keys.show');
